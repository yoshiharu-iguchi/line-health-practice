...
- `.env.example`：環境変数の設定名だけを書く見本（秘密の値は書かない）

## 秘密情報と環境変数

`.env.example` は、どの設定名が必要かを知らせる見本です。GitHubへ送ってもよいのは、秘密の値を書かないためです。

実際の `.env` は、将来、接続用の秘密情報を自分のPCや本番サーバーだけに置くためのファイルです。`.env` は `.gitignore` によりGitHubへ送られません。

この練習用サーバーは、環境変数 `PORT` が設定されていればその番号で起動し、設定がなければ `3000` を使います。秘密情報はまだ使用しません。
